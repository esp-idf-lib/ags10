...
esp_err_t ags10_set_i2c_address(i2c_dev_t *dev, uint8_t new_address)
{
    ...
    uint8_t buf[6] = { AGS10_SETADDR_REG,  new_address, ~new_address, new_address, ~new_address, 0 };
    buf[5] = crc8(buf + 1, 4);

    I2C_DEV_TAKE_MUTEX(dev);
    esp_err_t err = i2c_dev_write(dev, NULL, 0, buf, sizeof(buf));
    I2C_DEV_GIVE_MUTEX(dev);

    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to set new address: %d (%s)", err, esp_err_to_name(err));
        return err;
    }
...
esp_err_t ags10_set_zero_point_with(i2c_dev_t *dev, uint16_t value)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to set zero point: %d (%s)", err, esp_err_to_name(err));
        return err;
    }
